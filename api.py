...
def create_app() -> FastAPI:
    ...
    async def upload_file(
        ...
        if suffix not in ("md", "markdown", "txt", "docx", "pdf", "xlsx", "xls", "csv", "png", "jpg", "jpeg", "bmp", "tiff", "tif", "webp"):
            raise HTTPException(400, f"不支持的文件类型: .{suffix}（支持 .md/.txt/.docx/.pdf/.xlsx/.csv/.png/.jpg/.jpeg）")
        raw = await file.read()
        if not raw:
