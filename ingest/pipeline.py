...
class IngestPipeline:
    ...
    async def delete_source(self, source_id: str) -> tuple[int, list[str]]:
        ...
        顺序设计（先向量后 MySQL）：
          ...
          - 向量库删除失败直接抛异常 → DB 不删，关系库数据仍在，下次重试即可
          - 避免"关系库已删但向量残留"的孤儿场景
        
        跨库一致性：
          1. 标记 vector_synced=False（表示删除中）
          2. 向量库删除 chunks/titles/contents（失败则中断，不继续删 DB）
          3. MySQL 事务删除（含查孤儿实体 id）
          4. 孤儿实体向量删除（失败仅记录日志，不影响主流程）
        """
        # 1. 标记删除中
        await self.db.mark_vector_synced(source_id, False)

        # 2. 先删向量库（失败直接抛异常，DB 不动，用户重试）
        await self.vectors.adelete_by_source(source_id)

        # 3. MySQL 事务删除（含查孤儿实体 id）
        n, orphan_entity_ids = await self.db.delete_by_source(source_id)

        # 4. 删孤儿实体向量（DB 已提交，失败仅记录日志）
        if orphan_entity_ids:
            ...
            try:
                ...
                log.error("孤儿实体向量删除失败 ids={} err={}", orphan_entity_ids, e)
