...
| 数据库 | `pymysql` + `sqlalchemy` | MySQL 异步连接 |
| 日志 | `loguru` | 控制台 + 文件双输出 |
...
以下 3 类配置必须填写，否则服务无法启动（缺失时启动报错并列出缺失项）：
...
# AISAG_RECONCILE_INTERVAL=300            # 向量-MySQL 对账间隔秒数（默认 300，0=仅启动时）
...
- 首次启动自动执行：MySQL 建表 → 启动定时对账任务（每 `reconcile_interval` 秒）
...
| DELETE | `/api/documents/{id}` | 删除文档（MySQL 软删 + 向量硬删 + 对账兜底） |
...
| `AISAG_RECONCILE_INTERVAL` | 300 | 向量-MySQL 对账间隔（秒） |
...
- 有 MySQL 无向量的孤儿数据（补向量）
- 有向量无 MySQL 的孤儿向量（删向量）
...
默认在 `ai_sag/.chroma/` 目录，包含 5 个 collection：
