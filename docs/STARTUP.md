...
└── ai_sag/                 ← Python 包（含 __init__.py，即项目根目录）
    ...
    ├── splitter/           ← 文档切分
    ├── extractor/          ← 事件 & 实体抽取
    ├── vector_store/       ← 向量库封装（Chroma/FAISS/Milvus/PGVector）
    ├── storage/            ← 关系库持久化（MySQL/PG）
    ├── llm/                ← LLM 调用封装
    ├── loader/             ← 文件加载（md/txt/docx/pdf/xlsx/csv）
...
| 向量库 | `chromadb` | 向量存储与检索（5 套集合） |
| 数据库 | `aiomysql` + `asyncpg` | MySQL / PostgreSQL 异步连接 |
| 日志 | `loguru` | 控制台 + 文件双输出 |
| 配置 | `python-dotenv` | 从 `.env` 加载环境变量 |
| 上传 | `python-multipart` + `aiofiles` | 文件上传解析 |
| 文档解析 | `pypdf` + `python-docx` + `openpyxl` | PDF/Word/Excel 解析 |
| OCR | `rapidocr-onnxruntime` | 图片文字识别（默认引擎） |

### 1.3 数据库

默认使用 MySQL，也可切换为 PostgreSQL（在 `.env` 中设置 `AISAG_DB_BACKEND=postgresql`）。
...
表结构由 `ensure_schema()` 在首次启动 API 时自动创建（MySQL 用 `schema.sql`，PG 用 `schema_pg.sql`），无需手动建表。
...
```ini
# ============================================================
# 1. 关系数据库（MySQL 或 PostgreSQL 二选一）
# ============================================================
AISAG_DB_BACKEND=mysql            # mysql（默认）| postgresql

# MySQL（AISAG_DB_BACKEND=mysql 时填写）
SAG_MYSQL_HOST=localhost
SAG_MYSQL_PORT=3306
SAG_MYSQL_DATABASE=sag            # 需手动创建
SAG_MYSQL_USER=root
SAG_MYSQL_PASSWORD=your_password

# PostgreSQL（AISAG_DB_BACKEND=postgresql 时填写）
...
# 2. 向量库后端
# ============================================================
# chroma（默认）| faiss | milvus | pgvector
...
# AISAG_DB_BACKEND=mysql                  # 关系库后端（默认 mysql）| postgresql
# AISAG_VECTOR_STORE_BACKEND=chroma       # 向量库后端（默认 chroma）| faiss | milvus | pgvector
...
- 首次启动自动执行：关系库建表（MySQL 用 `schema.sql`，PG 用 `schema_pg.sql`）→ 启动定时对账任务（每 `reconcile_interval` 秒）
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/documents` | 上传文档（文件：md / txt / docx / pdf / xlsx / csv） |
...
| 组件 | 配置项 | 可选值 |
|------|--------|--------|
| 关系数据库 | `AISAG_DB_BACKEND` + `SAG_MYSQL_*` / `SAG_PG_*` | `mysql`（默认）/ `postgresql` |
| 向量库 | `AISAG_VECTOR_STORE_BACKEND` | `chroma`（默认）/ `faiss` / `milvus` / `pgvector` |
