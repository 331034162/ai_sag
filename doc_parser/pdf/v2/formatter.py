...
def print_summary(result: PDFResult):
    ...
    if result.tables:
        ...
        for tbl in result.tables:
            info = f"  - 表格 {tbl.table_index} (第{tbl.page_num}页): " \
                   f"{tbl.row_count}行 × {tbl.col_count}列"
            if tbl.title:
                info += f" [{tbl.title}]"
            if tbl.form_fields:
                info += f", {len(tbl.form_fields)}个表单字段"
            if tbl.signing_info:
                info += f", {len(tbl.signing_info)}条签章信息"
            print(info)
...
def print_page_text(result: PDFResult, max_chars: int = 500):
    ...
    for page in result.pages:
        ...
        for tbl in page.tables:
            print(f"\n  [表格 {tbl.table_index}]")
            if tbl.title:
                print(f"  标题: {tbl.title}")
            lines = tbl.markdown_text.strip().split('\n')
