...
| 阶段 | 做了什么 | 不做什么 |
|------|---------|---------|
| **离线（入库）** | 文档切分 → 事件抽取 → 实体识别 → 建事件-实体多对多索引（MySQL/PG） + 向量化（Chroma/FAISS/Milvus） | 不建知识图谱、不预计算关联路径 |
...
- **全链路异步**：MySQL（aiomysql）/ PostgreSQL（asyncpg）+ Embedding + LLM 全异步并发
...
| MySQL 或 PostgreSQL | MySQL ≥ 8.0 / PG ≥ 13 | 关系数据存储（任选其一） |
...
表结构会在首次启动 API 时**自动创建**（MySQL 用 `schema.sql`，PG 用 `schema_pg.sql`），无需手动建表。
...
| 变量 | 说明 | 示例 |
|------|------|------|
| `AISAG_DB_BACKEND` | 关系库后端（`mysql` / `postgresql`） | `mysql` |
| `SAG_MYSQL_PASSWORD` | MySQL 密码（使用 MySQL 时必填） | `your_password` |
| `SAG_PG_PASSWORD` | PostgreSQL 密码（使用 PG 时必填） | `your_password` |
| `AISAG_VECTOR_STORE_BACKEND` | 向量库后端（`chroma` / `faiss` / `milvus` / `pgvector`） | `chroma` |
...
| **向量数据库** | ChromaDB（默认）/ FAISS / Milvus / PGVector | ChromaDB ≥ 1.0.0 |
| **关系数据库** | MySQL + aiomysql / PostgreSQL + asyncpg | MySQL ≥ 8.0 / PG ≥ 13 |
...
| 组件 | 可选后端 | 配置方式（`.env` 中） |
|------|---------|----------------------|
| 关系数据库 | `mysql`（默认）/ `postgresql` | `AISAG_DB_BACKEND` + `SAG_MYSQL_*` / `SAG_PG_*` |
| 向量库 | `chroma`（默认）/ `faiss` / `milvus` / `pgvector` | `AISAG_VECTOR_STORE_BACKEND` |
...
└── ai_sag/
    ...
    ├── splitter/               ← 文档语义切分
    ├── extractor/              ← 事件 & 实体抽取
    ├── vector_store/           ← 向量库封装（Chroma/FAISS/Milvus/PGVector，5 个 collection）
    ├── storage/                ← 关系库持久化（MySQL/PG，6 张表，全异步）
