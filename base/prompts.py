...
RERANK_PROMPT_TEMPLATE = (
    ...
    "基于该实体在事件中的关联权重计算（强=核心关联，弱=背景引用）。"
    "行尾「强X弱Y」汇总了命中质量。优先选择强信号命中多的候选事件；"
    "仅命中弱信号（强0弱>0）的事件应降低排名，即便命中数多也不代表强相关；\n"
    "4) 实体命中数是强约束：命中查询实体数为 0 的事件应大幅降低排名，"
    "除非标题/摘要与查询主题高度匹配；\n"
    "5) 参考向量相关度：每个候选标注了「向量相关度」分数（0~1，越高越相关），"
    "该分数基于事件内容与查询的向量相似度计算。分数高支持相关性判断，"
    "但主题不符时仍应降低排名——向量分数是辅助信号，不是决定性信号。"
)
...
def query_extract_system_prompt() -> str:
    ...
    return (
        ...
        f"{ENTITY_NAME_NORM}"
        f"保留查询中明确提及的时间/金额等限定性实体（如'2024年''100万'）作为检索约束；"
        f"但不要从查询中推断或扩展出未明确提及的数值/日期。"
    )
